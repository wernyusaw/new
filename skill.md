...
- tests: unit/api tests, organized per feature (see section 6a for the full layout)
...
## 6a) Test Structure
Tests live under tests/, one folder per feature (mirrors src/ layer split, not a copy of it):
- tests/calculate/
  - calculate.api.test.ts (supertest end-to-end)
  - CalculateController.test.ts, CalculateService.test.ts
  - CalculateRequestMapper.test.ts, CalculateUsecase.test.ts
- tests/greeting/
  - greeting.api.test.ts
  - GreetingController.test.ts, GreetingService.test.ts, formal-greeting.service.test.ts
  - GreetingRequestMapper.test.ts, GetGreetingMessageUsecase.test.ts
  - MySqlGreetingRepository.test.ts, NoopGreetingRepository.test.ts
- tests/hello/
  - HelloController.test.ts, HelloService.test.ts
  - HelloRequestMapper.test.ts, GetHelloMessageUsecase.test.ts
- tests/profile/
  - profile.api.test.ts
  - ProfileController.test.ts, ProfileService.test.ts
  - ProfileRequestMapper.test.ts, ProfileResponseMapper.test.ts
  - MySqlProfileRepository.test.ts, NoopProfileRepository.test.ts
- tests/common/
  - ApiResponseMapper.test.ts (shared response envelope mapper)

Conventions:
- Layer test files (controller/service/repository/mapper/usecase) use PascalCase matching the source class or module under test, e.g. ProfileController.ts -> ProfileController.test.ts.
- *.api.test.ts integration files keep the feature's kebab-case name (they test the route, not one class).
- Controller/service/repository tests mock every injected port with jest.fn(); MySql*Repository.test.ts mocks src/db/mysql.getMySqlPool.
- Use AAA comments (Arrange/Act/Assert) inside each `it`.
- *.api.test.ts files are integration tests using supertest against createApp() from Bootstrap.ts.
- coverage/ (generated by npm run test:coverage) is a build artifact, not checked in.
...
Tests:
- npm test
- npm run test:coverage (writes report to coverage/, target >= 80% per changed file)
- npm run test:watch
